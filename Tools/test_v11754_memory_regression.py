...
def main():
    ...
    for marker in [
        ...
        if marker not in src:
            raise RuntimeError(f"missing .54 memory/project/native marker: {marker}")

    base = Path(tempfile.mkdtemp(prefix="Vex11754MemoryRegression-"))
    ...
    try:
        ...
        # .54 extends the proven .53 supervisor without rewriting its internal
        # version identity. Prove both the .53 autolearn engine and .54 Windows route.
        ...
        native = wait_json(f"http://127.0.0.1:{port}/windows/capabilities?{query}", lambda d: d.get("ok") is True and d.get("version") == "0.11.7.54", 30, "windows native")
        ...
        print("[v11754-memory] PASS .51 write + .52 correction + natural Nyx recall under .54", flush=True)
    finally:
        stop(bp)
        try:
