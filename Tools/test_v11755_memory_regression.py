...
def main():
    ...
    for marker in [
        ...
        if marker not in src:
            raise RuntimeError(f"missing .55 memory/project/native marker: {marker}")

    base = Path(tempfile.mkdtemp(prefix="Vex11755MemoryRegression-"))
    ...
    try:
        ...
        # .55 wraps the source-grounded project supervisor with anti-spam guards
        # and advances both the autolearn and Windows-native status identities.
        auto = wait_json(
            f"http://127.0.0.1:{port}/autolearn/status?{query}",
            lambda d: d.get("ok") is True
            and d.get("version") == "0.11.7.55"
        ...
        print("[v11755-memory] PASS .51 write + .52 correction + natural Nyx recall under .55", flush=True)
    finally:
        stop(bp)
        try:
