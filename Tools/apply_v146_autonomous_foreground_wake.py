...
if "V146_AUTONOMOUS_FOREGROUND_WAKE" in text:
    print("PASS v0.14.6 autonomous foreground wake already applied")
    raise SystemExit(0)
...
replacement = '''struct VexVoiceCommandIntent: AppIntent {
    // V146_AUTONOMOUS_FOREGROUND_WAKE
    static var title: LocalizedStringResource = "Ask Vex"
    static var description = IntentDescription("Give Vex a spoken iPhone command.")
...
replacement = '''    @MainActor
    func perform() async throws -> some IntentResult & ProvidesDialog {
        ...
        switch await VexVoicePhoneExecutor.perform(command) {'''
if anchor not in text:
    raise SystemExit("v0.14.6 Ask Vex perform anchor missing")
text = text.replace(anchor, replacement, 1)
...
print("PASS v0.14.6 autonomous foreground wake patch")
