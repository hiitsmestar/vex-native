...
if "0.11.7.71" not in source:
    raise SystemExit("v0.11.7.77 expected .71 build identity missing")
source = source.replace("0.11.7.71", "0.11.7.77")
source = source.replace(
    'Vex-Agent-Runtime-v0.11.7.77-SelfRepair-NaturalRecall',
    'Vex-Agent-Runtime-v0.11.7.77-WindowControlBroker',
)
source = source.replace(
    'Vex Agent Runtime v0.11.7.77 Self Repair + Natural Grounded Recall',
    'Vex Agent Runtime v0.11.7.77 Window Control Broker',
)
...
if anchor not in source:
    raise SystemExit("v0.11.7.77 nested .71 patch anchor missing")
...
exec(compile(source, str(source_path) + "[v11777]", "exec"), globals_dict)
