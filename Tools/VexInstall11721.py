...
VERSION='0.11.7.21'
FILES=['VexBridge.exe','VexRemoteSupport.exe','VexDoctor.exe','VexBridgeWatchdog-v11721.ps1']
...
def verify_doctor(home:Path)->None:
    doc=home/'VexDoctor.exe'; out=Path(os.environ.get('APPDATA',str(Path.home())))/'VexDoctor'/'install-smoke.json'
    if not doc.exists(): raise RuntimeError('VexDoctor.exe was not installed.')
    out.parent.mkdir(parents=True,exist_ok=True)
    try: subprocess.run([str(doc),'--headless','--json-out',str(out)],cwd=str(home),timeout=45,capture_output=True,text=True)
    except Exception as exc: raise RuntimeError(f'VexDoctor smoke test could not run: {exc.__class__.__name__}')
...
def main():
    ...
    try:
        ...
        stop_all_vex(home)
        for name in FILES:
            dstname='VexBridgeWatchdog.ps1' if name.endswith('v11721.ps1') else name
        ...
        messagebox.showinfo('Vex Install',f'Vex {VERSION} installed. Local control starts before indexing and the watchdog now preserves Bridge startup/recovery time.\n\nStart a fresh 2-hour support session and enable SAFE maintenance.')
    except Exception as exc:
