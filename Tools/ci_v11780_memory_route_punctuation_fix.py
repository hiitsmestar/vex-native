...
if "0.11.7.71" not in source:
    raise SystemExit("v0.11.7.80 expected .71 build identity missing")
source = source.replace("0.11.7.71", "0.11.7.80")
source = source.replace(
    'Vex-Agent-Runtime-v0.11.7.80-SelfRepair-NaturalRecall',
    'Vex-Agent-Runtime-v0.11.7.80-MemoryRoutePunctuationFix',
)
source = source.replace(
    'Vex Agent Runtime v0.11.7.80 Self Repair + Natural Grounded Recall',
...
if anchor not in source:
    raise SystemExit("v0.11.7.80 nested .71 patch anchor missing")
...
exec(compile(source, str(source_path) + "[v11780]", "exec"), globals_dict)
