...
import hashlib, json, os, shutil, ssl, subprocess, sys, time, urllib.parse, urllib.request
from pathlib import Path
from tkinter import Tk, messagebox

VERSION='0.11.7.19'
FILES=['VexBridge.exe','VexRemoteSupport.exe','VexDoctor.exe','VexBridgeWatchdog-v11719.ps1']
...
def wait_bridge_version(expected:str, seconds:int=45)->None:
    cfg=bridge_config(); token=str(cfg.get('token') or '').strip(); port=int(cfg.get('port') or 8765)
    if not token: raise RuntimeError('Bridge config token is unavailable.')
    ctx=ssl._create_unverified_context(); deadline=time.time()+seconds; last='no response'
    url=f'https://127.0.0.1:{port}/status?'+urllib.parse.urlencode({'token':token})
    while time.time()<deadline:
        try:
        ...
        time.sleep(1)
    raise RuntimeError(f'New Bridge did not prove runtime version {expected}: {last}')
...
def verify_doctor(home:Path)->None:
    ...
    try:
        r=subprocess.run([str(doc),'--headless','--json-out',str(out)],cwd=str(home),timeout=45,capture_output=True,text=True)
    except Exception as exc:
...
def main():
    ...
    try:
        ...
        stop_all_vex(home)
        for name in FILES:
            dstname='VexBridgeWatchdog.ps1' if name.endswith('v11719.ps1') else name
        ...
        messagebox.showinfo('Vex Install',f'Vex {VERSION} installed and live-verified. Bridge, Remote Support, Doctor, and health-aware watchdog are installed.\n\nStart a fresh 2-hour support session and enable SAFE maintenance.')
    except Exception as exc:
