...
# This script runs after the v0.8.6 hands-free patch. The first mic tap in v0.8.6
# calls stopRecognition() before a tap has ever been installed, and AVAudioEngine
# can terminate the process when removeTap(onBus:) is called with no existing tap.
# Track tap lifecycle explicitly and unwind safely if audio-engine startup fails.
...
old_install = '''        input.installTap(onBus: 0, bufferSize: 1024, format: format) { [weak request] buffer, _ in\n            request?.append(buffer)\n        }\n\n        audioEngine.prepare()\n        try audioEngine.start()\n        isListening = true\n'''
new_install = '''        input.installTap(onBus: 0, bufferSize: 1024, format: format) { [weak request] buffer, _ in\n            request?.append(buffer)\n        }\n        inputTapInstalled = true\n\n        audioEngine.prepare()\n        do {\n            try audioEngine.start()\n        } catch {\n            if inputTapInstalled {\n                input.removeTap(onBus: 0)\n                inputTapInstalled = false\n            }\n            recognitionRequest?.endAudio()\n            recognitionRequest = nil\n            throw error\n        }\n        isListening = true\n'''
