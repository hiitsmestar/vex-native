...
def gh_json(endpoint: str, timeout: int = 45):
    raw = run_gh(["api", endpoint], timeout=timeout)
    return json.loads(raw)


def post_command(peer_node_id: str, action: str) -> str:
    command_id = f"peer-{action}-{uuid.uuid4().hex[:12]}"
    body = "VEXCMD " + json.dumps({
        "id": command_id,
...
def summarize_status(payload: dict) -> dict:
    ...
    return {
        ...
        "memory_total_gb": memory.get("memory_total_gb"),
        "memory_available_gb": memory.get("memory_available_gb"),
    }
...
def coordinate_once(state: dict, timeout_seconds: int = DEFAULT_TIMEOUT_SECONDS) -> dict:
    ...
    summary.update({
        ...
        "round_trip_seconds": round(time.time() - started, 2),
        "secondary_ready": bool(summary.get("bridge_reachable") and summary.get("cognition_ok")),
    })
...
def self_test() -> int:
    sample = {
        ...
        "payload": {
            ...
            "action": "status",
            "result": {
                "agent_version": "0.11.7.62",
                "bridge": {"reachable": True, "version": "0.11.7.39"},
                "cognition": {"ok": True, "model": "test-model"},
                "hardware": {"memory": {"memory_total_gb": 4.0, "memory_available_gb": 1.5}},
            },
        },
    }
    body = "VEXRESULT\n```json\n" + json.dumps(sample, indent=2) + "\n```"
    parsed = parse_vexresult(body)
    assert parsed and parsed["payload"]["command_id"] == "abc"
    summary = summarize_status(parsed["payload"])
    assert summary["bridge_reachable"] is True
    assert summary["cognition_ok"] is True
    assert summary["bridge_version"] == "0.11.7.39"
    print("VexPeerCoordinator self-test OK")
    return 0
...
def main() -> int:
    ...
    if args.once:
        try:
            ...
            print(f"peer coordination failed: {exc.__class__.__name__}", file=sys.stderr)
            return 2
    while True:
        try:
